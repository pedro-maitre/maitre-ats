  ...
  return (
    <div className="max-w-3xl mx-auto animate-in fade-in duration-500">
      <div className="mb-8 flex items-center gap-4">
        <Link href={`/candidates`} className="p-2 rounded-full hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-500 transition-colors">
          <ArrowLeft size={20} />
        </Link>
        <div>
          <h1 className="text-3xl font-bold text-slate-900 dark:text-white">Cadastrar Candidato</h1>
          <p className="text-slate-500 dark:text-slate-400 mt-1">Insira os dados manualmente no Banco de Talentos.</p>
        </div>
      </div>

      <form action={createCandidate} className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800 overflow-hidden">
        ...
        <div className="bg-slate-50 dark:bg-slate-950/50 p-6 border-t border-slate-200 dark:border-slate-800 flex justify-end gap-4">
          <Link href={`/candidates`} className="px-6 py-2.5 rounded-lg font-medium text-slate-600 hover:text-slate-900 dark:text-slate-400 dark:hover:text-white transition-colors">
            Cancelar
          </Link>
          <SubmitButton text="Cadastrar" loadingText="Cadastrando..." />
        </div>
      </form>
    </div>
  );
