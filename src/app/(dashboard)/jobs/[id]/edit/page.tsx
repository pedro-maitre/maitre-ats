...
import { Briefcase, Building2, MapPin, AlignLeft, ArrowLeft, Loader2 } from "lucide-react";
...
export default async function EditJobPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // Create an updateJob function with bound ID for the server action
  const updateJobWithId = updateJob.bind(null, id);

  return (
    <div className="max-w-3xl mx-auto animate-in fade-in duration-500">
      <div className="mb-8 flex items-center gap-4">
        <Link href={`/jobs/${id}/board`} className="p-2 rounded-full hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-500 transition-colors">
          <ArrowLeft size={20} />
        </Link>
        <div>
          <h1 className="text-3xl font-bold text-slate-900 dark:text-white">Editar Vaga</h1>
          <p className="text-slate-500 dark:text-slate-400 mt-1">Atualize os detalhes da oportunidade.</p>
        </div>
      </div>

      <form action={updateJobWithId} className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800 overflow-hidden">
        ...
        <div className="bg-slate-50 dark:bg-slate-950/50 p-6 border-t border-slate-200 dark:border-slate-800 flex justify-end gap-4">
          <Link href={`/jobs/${id}/board`} className="px-6 py-2.5 rounded-lg font-medium text-slate-600 hover:text-slate-900 dark:text-slate-400 dark:hover:text-white transition-colors">
            Cancelar
          </Link>
